...
BUILDING_BLURB = {
    ...
    resources.WOODCUTTERS_CAMP:
        "Sends cutters out to woodland in this region and hauls the timber "
        "home. The best stands are rarely the ones you live in.",
    resources.GRANGE:
        "An outfarm on good ground elsewhere in this region, worked from "
        "here. Reaches fields your own doorstep cannot.",
    resources.WORKINGS:
...
def _outstation_verdict(world, node, building):
    """Judged on the ground this REGION has of the right kind and how much of
    it is already being worked -- not on the village's own stock or its own
    catchment, which is exactly the reading that kept the mining tier
    invisible (almost no village sits on ore, so nothing about a village's own
    land ever suggested a mine). The same reading would hide every other
    member of the family for the same reason.

    One function for the whole family (phase D): the judgement is identical
    whatever kind of country it is, which is the point."""
    label = resources.OUTSTATIONS[building][2]
    region = world.regions[node.region_id]
    total = resources.region_outstation_cells(world, region, building)
    if total <= 0:
        return "idle", f"No {label} land in this region to work.", 0.0
    camps = sum(1 for vid in getattr(region, "villages", [])
                if 0 <= vid < len(world.villages)
                and resources.storage_tier(world.villages[vid], building) > 0)
    # Scores are compared against every other building's inside a priority
    # band, and the rest of them are 0..1 fractions (see _storage_verdict's
    # fill). Returning a raw cell count here made an outstation outrank a
    # granary that was 100% full and turning production away, purely because
    # 182 > 1.0. Normalised to "how much of a tier-1 camp could this region
    # actually keep busy", which is the same shape as a fill fraction.
    per_camp = total / max(1, camps + 1)
    tier1 = resources.OUTSTATION_CELLS[1] or 1
    score = min(1.0, per_camp / tier1)
    if camps == 0:
        return ("urgent",
                f"{total:,} {label} cells in this region and nobody working "
                f"them.",
                score)
    worked = resources.outstation_cells(world, node, building)
    if worked >= total / max(1, camps):
        return ("idle",
                f"This region's {total:,} {label} cells are already shared "
                f"between {camps} camps.", 0.0)
    return ("useful",
            f"{total:,} {label} cells, shared between {camps} camp"
...
def _storage_effect_lines(node, building, to_tier):
    ...
    if building in resources.OUTSTATIONS and to_tier is not None:
        table = resources.OUTSTATION_CELLS
        label = resources.OUTSTATIONS[building][2]
        if to_tier < len(table):
            now = table[min(resources.storage_tier(node, building), len(table) - 1)]
            lines.append(f"Works {table[to_tier]} {label} cells in this region "
                         f"(now {now})")
