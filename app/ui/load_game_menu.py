"""Load Game popup: lists saves on disk, pick one, then Load or Cancel."""
...
class LoadGameMenuView(tk.Frame):
    def __init__(self, master, on_load, on_cancel):
        ...
        tk.Button(actions, text="Cancel", command=self._cancel, width=12,
    ...
    def refresh(self, saves):
        ...
        for meta in saves:
            ...
            for widget in (row, lbl):
                widget.bind("<Button-1>", lambda e, sid=save_id: self._select(sid))
            self._rows[save_id] = (row, lbl)

    def _select(self, save_id):
