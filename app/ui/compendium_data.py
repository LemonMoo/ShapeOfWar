...
def _settlements_article():
    ...
    parts = [
        ...
        (
            ...
            "yet. From there population moves BOTH ways. A node that is "
            "fed and sheltered grows toward its ceiling, closing a small "
            "fraction of the remaining gap each turn (only while it's "
            "not in a Food/Firewood shortage grace period) — for an "
            "established City this genuinely takes decades of in-game "
            "time to meaningfully close, by design, while frontier "
            "communities (Villages and Towns) grow 40x as fast. On top "
            "of that natural increase, people migrate: a settlement that "
            "clearly out-earns its region's and its kingdom's average "
            "wealth attracts people and grows faster, while one that is "
            "clearly poorer than its neighbours sheds people and "
            "declines toward the floor. That relative-wealth pull is "
            "what keeps the pyramid of Villages, Towns and Cities "
            "spread the way a real realm is: only the settlements that "
            "genuinely thrive in their surroundings ever grow populous "
            "enough to be raised to a Town, or a Town to a City — the "
            "rest stay put, and a settlement that keeps losing people "
            "eventually falls back a rung (a City back to a Town, a "
            "Town back to a Village). Population can never be driven "
            f"below {_pct(R.POPULATION_MIN_FRACTION)} of its own max "
            "population by shortage or emigration — a hard-scrabble "
            "remnant survives rather than the settlement being wiped "
            "out entirely. And a remnant is not permanently crippled: "
            "children grow into the missing adults over time, so a "
            "village that lost every adult to a hard Winter or an "
            "expansion slowly rebuilds its workforce."
        ),
        (
...
def _construction_article():
    ...
    parts = [
        ...
        (
            ...
            "underground) — builds in stone instead, and the rule follows "
            "the tier progression: every timber-derived line in a building "
            "cost is paid as Stone — Logs at "
            f"{construction.DRY_STONE_LOG_RATIO:g} Stone per Log, Planks "
            f"(the manufactured timber of tier-2+ buildings) at "
            f"{construction.DRY_STONE_PLANK_RATIO:g} Stone per Plank, "
            "exactly their value on the resource ladder. The same way such "
            "a region burns coal for heat in Winter, its stone is its "
            "timber, so storage, camps and towns stay buildable at every "
            "tier rather than deadlocking on a forest it does not have — "
            "but dressed stone weighs more than timber, so the cost in "
            "tonnage is real. Bricks stay bricks (the rule substitutes "
            "timber, not clay), and ships are the exception: a hull is "
